...
> ec-shop アプリの Prometheus メトリクスは **`ec_` 接頭辞**付きで、サービスラベルは **`service=`** (`app=` ではない)、HTTP ステータスは **`status=`** (`code=` ではない). 詳細は `environment.md` 参照.
...
**LogQL 起点** — `{namespace="ec-shop", service="ec-web"}` から始め、`|= "error"` か `|~ "(?i)(timeout|exception|panic)"` で絞り、`| json` で構造化、最後に `| line_format` で読みやすく。`| limit 200` を必ず付けてトークンを節約する。
